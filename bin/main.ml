...
let () =
  ...
  in
  let usage_msg = "usage: ./panda [-a|-s|-l|-c] [file.pd]" in
  ...
  | _ -> (
      let sast = Panda.Semantic.check ast in
      match !action with
      | Ast -> ()
      | Sast -> print_string (Panda.Sast.string_of_sprogram sast)
      | LLVM_IR ->
          print_string (Llvm.string_of_llmodule (Panda.Irgen.translate sast))
      | Compile ->
          let m = Panda.Irgen.translate sast in
